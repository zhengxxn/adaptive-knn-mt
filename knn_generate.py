...
def _main(args, output_file):
    ...
    # add by , we put code for load knn store hear
    if args.use_knn_store:
        knn_store = KNN_Dstore(args=args, trg_vocab_size=task.tgt_dict.__len__())
    else:
        knn_store = None
    # end
    ...
    # add by , here we transfer the use knn store parameter to build generator
    extra_gen_cls_kwargs = {"lm_model": lms[0], "lm_weight": args.lm_weight,
                            "use_knn_store": args.use_knn_store, "knn_store": knn_store,
                            "knn_coefficient": args.knn_coefficient, "check-knn-result": args.check_knn_result}
    ...
    if args.use_knn_store:
        print(
            'average time for retrieve neighbors, {} s'.format(knn_store.time_for_retrieve / knn_store.retrieve_count))
        print('average time for set the target prob for each neighbor'
              ' (need do scatter operation for (batch size * beam size * k, vocab size) tensor), and k is {},  {} s'
              .format(args.k, knn_store.time_for_setup_prob / knn_store.retrieve_count))
